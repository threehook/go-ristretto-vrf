...
func TestHonestComplete(t *testing.T) {
	alice := []byte("alice")
	pk, sk := GenerateKeys()
	aliceVrf := NewVRF(alice, sk)
	//aliceVRFFromProof, aliceProof := aliceVrf.Prove(alice, sk)
	aliceVRFFromProof := aliceVrf.Prove(alice, sk)

	fmt.Printf("pk:           %X\n", pk)
	fmt.Printf("sk:           %X\n", sk)
	fmt.Printf("alice(value): %X\n", alice)
	fmt.Printf("aliceVRF:     %X\n", aliceVrf.value)
	fmt.Printf("aliceProof:   %X\n", aliceVrf.proof)

	if !aliceVrf.Verify(pk, alice) {
		t.Errorf("Gen -> Compute -> Prove -> Verify -> FALSE")
	}

	if !bytes.Equal(aliceVrf.value[:], aliceVRFFromProof[:]) {
...
func TestVerify(t *testing.T) {
	var vrf VRF
	var pk = [Size]byte{0xBA, 0xF5, 0x43, 0xCF, 0xA7, 0x61, 0xAB, 0x43, 0xB6, 0xC8, 0x10, 0x40, 0x20, 0xDD, 0x07, 0x4B, 0x90, 0x7B, 0x96, 0x44, 0xA3, 0x15, 0x05, 0xE1, 0x1F, 0x0B, 0x6B, 0xAE, 0x47, 0x32, 0xAC, 0x7C}
	alice := []byte{0x61, 0x6C, 0x69, 0x63, 0x65}
	var aliceVRF = [Size]byte{0x49, 0xE1, 0x12, 0x4E, 0xE9, 0x76, 0x1A, 0x7A, 0xDC, 0xF8, 0xC3, 0x3C, 0x0D, 0x33, 0xF6, 0x20, 0x7E, 0x5C, 0xE6, 0xAB, 0xD5, 0xF6, 0xFE, 0xAD, 0x73, 0xD0, 0xBC, 0x84, 0xB1, 0xAB, 0xE9, 0xCF}
	var aliceProof = [ProofSize]byte{0x78, 0x3E, 0xBA, 0xBA, 0xFD, 0xD7, 0x0F, 0xB9, 0x21, 0x6C, 0x3F, 0xE3, 0xF7, 0xD6, 0x52, 0x89, 0x24, 0xED, 0x93, 0x32, 0x52, 0x02, 0x91, 0x93, 0xDB, 0x58, 0x55, 0x36, 0x57, 0x6B, 0x05, 0x01, 0x44, 0x50, 0x40, 0x37, 0x2F, 0x06, 0x82, 0x75, 0x58, 0xC3, 0xE8, 0x7A, 0x9A, 0x68, 0x4D, 0x83, 0x98, 0x63, 0x59, 0x3B, 0x57, 0xD3, 0xBD, 0xC0, 0x9F, 0x09, 0x42, 0x21, 0xD8, 0xBF, 0x20, 0x08, 0x00, 0xF5, 0x01, 0xC3, 0xE8, 0xE4, 0xF4, 0xBC, 0xDD, 0xD9, 0xE2, 0x2D, 0x59, 0x00, 0xE7, 0x1C, 0xC8, 0x33, 0x64, 0xC2, 0x7C, 0x03, 0xB4, 0x67, 0xFB, 0x9A, 0x7E, 0x0B, 0x8D, 0xC0, 0x99, 0x22}
	vrf.value = aliceVRF
	vrf.proof = aliceProof

	if !vrf.Verify(pk, alice) {
		t.Errorf("Gen -> Compute -> Prove -> Verify -> FALSE")
	}
}
